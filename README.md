...
| 手動任務重新命名 | 手動新增任務時，「影片鏈接」下方的「更改名稱(可空)」輸入框填入想要的名稱 | 原理與 sn_list.txt 的 `<重新命名>` 標籤相同，留空用預設名稱，填寫後單集／最新一集／最後一集／全部／掃描集數勾選下載都會套用，不用先把作品加進 sn_list.txt 才能改名 |
...
| **每日新番通知**（v25.2.0 新增） | 「設定」→「通知設定」→「通知類別」新增開關（預設開啟），每天 00:00 自動彙整並發送一則通知：本日依 `sn_list.txt` 自訂排程要更新的作品清單（附線上查詢的預計集數）＋監視公告機動調整今天生效中的異動，皆可透過「通知模板」自訂文字格式 | v24.6 與本客製版 v25.1.9 以前完全沒有主動彙整的概念，只能等各別事件通知一則則累積；本功能一次看到「今天有哪些番要更新」，不用自己對照 sn_list.txt 回想 |
...
| `sn_list.txt` 完整資料庫化（v25.2.0 調整） | 無需操作，`sn_list.txt` 改逐行存進 `aniGamer.db`（追蹤項目／單獨註解行／空白分隔行／星期標籤各自一列），排程頁面文字編輯器仍以 `sn_list.txt` 格式的原始文字為權威顯示與編輯介面 | 搭配下方 aniGamer.db 資料表整理一併達成「每一列資料都能直接用 DB 瀏覽器看懂、直接修改」，不用再解一份整包塞成字串的欄位 |
...
| sn 交集比對，不比對名稱字面 | 無需操作，比對作品時透過 `search.php` 查詢＋比較完整集數 sn 清單是否有交集 | 公告、搜尋結果、sn_list.txt 三邊的名稱經常不一致（例如站方加了 `[年齡限制版]`，或用 `2nd Season` 代替 `第二季`），純比名稱字面容易誤判或漏判，sn 交集判斷不受名稱寫法影響 |
| 動態調整檢查時間，不更動 sn_list.txt | 「機動調整時間」開啟後才會實際套用，暫停/延後/合併集數狀況都只寫入獨立的臨時檔案 | sn_list.txt 本身完全不會被程式自動改寫，使用者手動維護的排程設定不會被覆蓋或搞亂；合併集數／加更兩集時還有雙重保險，30 分鐘後自動複查是否已補齊剩餘集數 |
