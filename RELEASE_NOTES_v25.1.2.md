...
- 新增 `ja3`／`akamai` 自訂指紋支援：可在 Dashboard 設定頁或 `config.json` 填入自己瀏覽器實際採集到的指紋（取代 curl_cffi 內建的罐頭指紋），需與取得 cookie 的瀏覽器一致；`ja3` 與 `akamai` 須同時填寫才會生效，只填一個會警示並自動退回內建指紋，不影響現有使用者
