...
| **快速取得 Cookie／UA／JA3／Akamai**（v25.1.8 新增） | 「設定」頁「反爬蟲設定」區塊「快速取得」按鈕：「Cookie」會開一個乾淨的獨立瀏覽器視窗導向動畫瘋首頁，登入後按視窗上的獨立控制面板按鈕即自動填回欄位；「UA／JA3／Akamai」則導向 ja3.zone/check，等頁面載入完成按按鈕即三個欄位一次擷取填入 | 不用再自己開發者工具複製 Cookie、或手動前往 ja3.zone/check 逐一複製三個欄位；獨立乾淨的瀏覽器設定檔不受你平常瀏覽器登入狀態干擾，且三者保證來自同一個瀏覽器核心指紋一致；打包後的 `.exe` 也能直接使用 |
