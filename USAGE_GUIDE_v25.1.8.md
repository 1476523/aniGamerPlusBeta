...
- 專案根目錄仍保留官方原版格式的 `config-sample.json`／`sn_list-sample.txt` 作為格式參考，這兩個檔案**不會被程式讀取**，純粹只是留給需要對照官方格式的使用者查閱。
...
接著到「反爬蟲設定」區塊，填寫 Cookie／JA3／Akamai（需要登入巴哈動畫瘋會員才能下載付費/會員限定內容時才必要）。**建議直接用「快速取得」按鈕**，不用自己開發者工具找 Cookie：
...
填好後按「提交」即會**直接存進資料庫**，不需要自己建立或編輯 `sn_list.txt`。完整語法說明（分類、下載模式、自訂星期/時間等）請參考 README 中的〔[`sn_list.txt` 排程功能強化](README.md)〕章節，用法完全一致，只是儲存位置改成資料庫而已。
...
- **內容是重建出來的，不是原始檔案**：匯出的 `config.json`／`sn_list.txt` 是程式即時把資料庫內容轉換回檔案格式產生的，建議下載後對照專案根目錄附的官方格式參考檔 [`config-sample.json`](config-sample.json)／[`sn_list-sample.txt`](sn_list-sample.txt)，確認 `bangumi_dir`、`temp_dir` 等關鍵欄位內容正確無誤。
