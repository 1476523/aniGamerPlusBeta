...
> 只想看 v25.1.8 這次版本的變更清單，請參閱 [RELEASE_NOTES_v25.1.8.md](RELEASE_NOTES_v25.1.8.md)。
...
> ⚠️ **`ffmpeg.exe` 需要自行另外準備**：下載影片實際上是靠 `ffmpeg` 合併每個片段，<br>
> 但這個檔案體積較大（約 150MB），並**沒有**包在發布的壓縮檔裡。<br>
> 你需要自行下載 [ffmpeg](https://ffmpeg.org/download.html)，並選擇下面**其中一種**方式讓程式找得到它：<br>
...
程式下載影片時是透過 `ffmpeg` 把逐段影片合併成完整檔案，但這個工具**不包含在發布的壓縮檔裡**（體積約 150MB，故意不隨程式一起發布）。<br>
